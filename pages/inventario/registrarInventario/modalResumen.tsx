        ...
        <section className='flex flex-col gap-2 w-[50%] h-auto m-0 pl-6'>
            <div className='rounded-xl p-2 border-2 border-default-200 w-full h-full'>
                ...
                <div className='text-sm'>
                    <p className='text-sm truncate'>{jsonDataArticles ? `${jsonDataArticles.dependencia}: ${jsonDataArticles.saldo}` : 'Sector: Cantidad'}</p>
                    <p className='text-sm truncate'>{jsonDataArticles ? `Sin Asignar: ${jsonDataArticles.saldo_restante}` : 'Sin Asignar: Disponible'}</p>
                </div>
            </div>
            <div className='rounded-xl p-2 border-2 border-default-200 w-full h-[95px]'>
                ...
                <div className='text-sm'>
                    <p className='text-sm truncate'><FontAwesomeIcon icon={faWarehouse} className='text-xs mr-1 w-4'/>{jsonDataArticles ? `${jsonDataArticles.articulo} de ${jsonDataArticles.dependencia}` : 'Articulos de sector'}</p>
