    ...
    return (
        <div className='w-full my-[50px] h-[110px] bg-background-200 flex justify-around p-5 rounded-lg shadow-md'>
            <div className='flex flex-col'>
                ...
                <Select isDisabled={jsonIsLoading} aria-labelledby='label-dependency' value={valueSelects.deps} onChange={(e: any) => {setValueOption({value: e.value, type: 'Deps'}); handleValueSelect('deps', e)}} defaultValue={options.deps[0]} options={options.deps} className='w-[170px]' placeholder='Dependencias' noOptionsMessage={({ inputValue }) => !inputValue ? 'Sin opción' : 'Sin opción'} isSearchable styles={colourStyles}></Select>
            </div>
            <div>
            ...
            <div className='flex flex-col'>
                ...
                <Select isDisabled={jsonIsLoading || disabledClassrooms} aria-labelledby='label-curse' value={valueSelects.grades} onChange={(e: any) => {setValueOption({value: e.value, type: 'Grades'});  handleValueSelect('grades', e)}} options={options.grades} className='w-[170px]' placeholder='Cursos' noOptionsMessage={({ inputValue }) => !inputValue ? 'Sin opción' : 'Sin opción'} isSearchable styles={colourStyles}></Select>
            </div>
        </div>
    )
}
