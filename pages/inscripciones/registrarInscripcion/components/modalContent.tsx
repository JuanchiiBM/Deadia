            ...
            <section className='pr-6 border-r-1'>
                ...
                <div>
                    ...
                    <div className='flex gap-2 mb-2 mt-8'>
                        ...
                        <Select maxMenuHeight={140} value={dataForm.curse} onChange={(newValue: any) => selectOptionOfCurse(newValue, dataForm.category.discount)} isClearable isDisabled={!update && curseDisabled} className='w-[33%]' options={options.grade} placeholder='Curso' noOptionsMessage={({ inputValue }) => !inputValue ? 'Sin opción' : 'Sin opción'} isSearchable styles={colourStylesBordered} required></Select>
                    </div>
                    <div className='flex gap-2'>
                        <Select value={dataForm.amount} onChange={(newValue: any) => handleInputChange('amount', newValue)} isDisabled={optionsAmount == undefined} className='m-0 self-end w-[50%]' options={optionsAmount} menuPlacement="top" placeholder='Monto' noOptionsMessage={({ inputValue }) => !inputValue ? 'Sin opción' : 'Sin opción'} isSearchable styles={colourStylesBordered} required></Select>
                        <I18nProvider locale='es-ES'>
                            <DateRangePicker visibleMonths={2} value={dataForm.datePicker} isDisabled={!update && isDisabled} onChange={(e) => { handleInputChange('datePicker', e) }} id='datepicker' variant='bordered' label='Duración del Curso' labelPlacement='outside' className="max-w-xs w-[50%] transition-all" classNames={{
