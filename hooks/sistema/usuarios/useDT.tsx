            ...
            acciones: () => {
                return ReactDOMServer.renderToString(
                    <div id={`actions-${dato.id}`} style={{ display: "flex", justifyContent: "center", gap: "10px" }}>
                        ...
                        {hasPermission(MODULES.MODULEUSER, ACTIONS.DELETE) && <button className="delete-btn btn-sigma" id={`delete-btn-${dato.id}`}> <FontAwesomeIcon icon={faTrashCan} className="text-2xl text-default-400" /> </button>}
                    </div>
                );
            }
